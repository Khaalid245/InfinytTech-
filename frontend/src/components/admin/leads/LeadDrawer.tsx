    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 w-full max-w-2xl bg-surface-light border-l border-border-primary shadow-2xl z-50 flex flex-col transform transition-transform duration-300">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          {/* Inquiry / Booking Scope Callout */}
          <section className="p-4 rounded-xl bg-amber-500/5 dark:bg-amber-500/10 border border-amber-500/20 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-amber-600 dark:text-amber-400 flex items-center gap-1.5">
                <Clock className="w-3.5 h-3.5" />
                {lead.source || 'Direct Website Submission'}
              </span>
              {lead.project_type && (
                <span className="text-[11px] font-medium px-2 py-0.5 rounded bg-amber-500/15 text-amber-700 dark:text-amber-300">
                  {lead.project_type}
                </span>
              )}
            </div>
